class SnapshotsController < ApplicationController
  ...
  def snapshot_params
    params.require(:snapshot).permit(:weight_kg, :activity_level, :goal_weight_kg, :predicted_time_weeks, :calorie_deficit_or_surplus_per_day, :gender, :dob)
  end
  ...
  def preprocess_virtual_params
    virtual_params.tap do |vp|
      vp[:calorie_deficit_or_surplus_per_day] = vp[:calorie_deficit_or_surplus_per_day].to_s.gsub(/[^\d-]/, '') if vp[:calorie_deficit_or_surplus_per_day].present?
    end
  end
